...
$product = [
    ...
    'stock' => '',
    'image' => 'default.jpg',
    'vendor_id' => SessionManager::getCurrentUserId(),
    'is_featured' => false
];
...
if ($isEditing) {
    ...
    // Check if product exists and belongs to current vendor
    if (!$loadedProduct || $loadedProduct['vendor_id'] != SessionManager::getCurrentUserId()) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['size'] > 0) {
        ...
        $uploadDir = ROOT_PATH . '/assets/images/products/';
        ...
        // Check permissions and make sure directory is writable
        if (!is_writable($uploadDir)) {
            ...
            $errors[] = 'Error uploading file: ' . ($error ? $error['message'] : 'Unknown error');
        }
    } else {
        ...
        if ($isEditing) {
            ...
            // Set default image for new products if no image uploaded
            $productData['image'] = 'default.jpg';
        }
    }
    
    // Debug info - Log the product data before submitting
    error_log("Product data before saving: " . print_r($productData, true));
    ...
    if (empty($errors)) {
        if ($isEditing) {
            ...
            // Make sure image field is set and passed to the createProduct method
        ...
        // Check if the result is an array with success information
        if (is_array($result) && isset($result['success'])) {
            if ($result['success']) {
                $message = $result['message'] ?? $successMessage;
                $alertType = 'success';
                
                // Set session message to show on products list page
                $_SESSION['product_message'] = $message;
                $_SESSION['product_alert_type'] = 'success';
                
                // Debug - Log success information
                error_log("Product saved successfully: " . print_r($result, true));
                ...
                error_log("Error saving product: " . print_r($result, true));
            }
        } else if ($result) {
            // For backward compatibility if $result is just a boolean or other value
...
<main class="container py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <a href="<?php echo BASE_URL; ?>presentation/vendor/products.php" class="btn btn-outline-secondary">
